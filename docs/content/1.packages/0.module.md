...
You must also add the module to the Nuxt config. As the eslint.config inherits from `./.nuxt/eslint.config.mjs` and this is built during the `"postinstall": "nuxt prepare",` step
